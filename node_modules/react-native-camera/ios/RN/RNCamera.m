...
- (void)updateAutoFocusPointOfInterest
{
    ...
    if ([self.autoFocusPointOfInterest objectForKey:@"x"] && [self.autoFocusPointOfInterest objectForKey:@"y"]) {
        ...
        if ([device isFocusPointOfInterestSupported] && [device isFocusModeSupported:AVCaptureFocusModeContinuousAutoFocus]) {
            ...
            if (!self.isFocusedOnPoint) {
//                RCTLogWarn(@"AutoFocusPointOfInterest is supported");
                NSLog(@"AutoFocusPointOfInterest is supported");
            ...
            NSLog(@"AutoFocusPointOfInterest not supported");
...
- (void)takePictureWithOrientation:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject{
    [self.sensorOrientationChecker getDeviceOrientationWithBlock:^(UIInterfaceOrientation orientation) {
        NSLog(@"is taking picture with orientation");
...
{
    NSLog(@"is taking uncompressed picture");
...
//        NSString* key = (NSString*)rawPixelFormatType;
//        NSNumber* value = [NSNumber numberWithUnsignedInt:availableRawFormat];
...
//    AVCapturePhotoSettings *settings = [AVCapturePhotoSettings photoSettingsWithFormat:@{key : value}];
    NSLog(@"!!!!!!!!!!!!!!!!!!!!");
    NSLog(@"%@",self.stillImageOutput.availableRawPhotoPixelFormatTypes);
    NSUInteger rawFormat = self.stillImageOutput.availableRawPhotoPixelFormatTypes.firstObject.unsignedIntegerValue;
...
{
    NSLog(@"is capturing output");
    NSLog(@"BOOL--b-->%s",photo.isRawPhoto?"YES":"NO");
...
//    NSData* data = [AVCaptureStillImageOutput jpegStillImageNSDataRepresentation:rawSampleBuffer];
//    UIImage* takenImage = [UIImage imageWithData:data];
//    UIImage* takenImage = [self imageFromSampleBuffer:rawSampleBuffer];
    NSData *data = [photo fileDataRepresentation];
    NSLog(@"%s byte len:%lu", __PRETTY_FUNCTION__, (unsigned long)[data length]);
...
//    CFDictionaryRef metaDict = CMCopyDictionaryOfAttachments(NULL, rawSampleBuffer, kCMAttachmentMode_ShouldPropagate);
//
...
////    if (!self.deviceOrientation) {
////        [self takePictureWithOrientation:options resolve:resolve reject:reject];
////        NSLog(@"take picture orientation");
////        return;
////    }
//
//    NSInteger orientation = [options[@"orientation"] integerValue];
...
//                NSMutableData * destData = [NSMutableData data];
//
//                CGImageDestinationRef destination =  ((__bridge CFMutableDataRef)destData, kUTTypeJPEG, 1, NULL);
//
//                // defaults to true, must like Android
//                bool writeExif = true;
//
...
- (void)startSession
{
 ...
 //stillimageoutput -> photooutput
        ...
        if ([self.session canAddOutput:self.stillImageOutput]) {
            ...
            self.stillImageOutput = self.stillImageOutput;
        }
